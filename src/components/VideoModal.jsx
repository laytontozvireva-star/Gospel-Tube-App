...
import { X, MessageSquare, ListVideo } from "lucide-react";
...
export default function VideoModal({ video, onClose, relatedVideos = [], onSelectRelated }) {
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-7xl max-h-[95vh] overflow-hidden rounded-2xl bg-white shadow-2xl flex flex-col lg:flex-row" 
        ...
        <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar bg-slate-50">
          ...
          <div className="flex lg:hidden items-center justify-between border-b border-slate-200 px-4 py-3 bg-white sticky top-0 z-10">
            <div className="min-w-0 pr-4">
              ...
              <p className="truncate text-sm font-semibold text-slate-700">{video.speaker || video.author}</p>
            </div>
            <button type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900" aria-label="Close player">
              <X size={21} />
            </button>
          </div>
